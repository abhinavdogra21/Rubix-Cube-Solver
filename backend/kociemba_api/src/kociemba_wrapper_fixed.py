...
Fixed version to handle UTF-8 decoding errors and improve error handling
"""
...
class KociembaSolverWrapper:
    ...
    def _safe_decode(self, result):
        """Safely decode C++ library result with error handling"""
        if not result:
            return None
        ...
        try:
            ...
            try:
                # Try latin-1 as fallback (preserves all byte values)
                decoded = result.decode('latin-1').strip()
                # Check if it looks like valid ASCII moves
                if all(c in "URFDLB'2 " for c in decoded):
                    return decoded
                else:
                    return None
            except:
        ...
        if self.lib:
            try:
                ...
                if not cube_string or len(cube_string) != 54:
                    return "Error: Invalid cube string length. Expected 54 characters."
                
                # Check if cube string contains only valid characters
                if not all(c in '012345' for c in cube_string):
                    return "Error: Invalid cube string format. Only digits 0-5 are allowed."
                
                result = self.lib.kociemba_solve(cube_string.encode('utf-8'))
                if result:
                    solved = self._safe_decode(result)
                    if solved and solved != "Error":
                        return solved
                    else:
                        return "Error: No solution found or invalid result"
                else:
    ...
    def solve_scramble(self, scramble: str) -> str:
        """Solve a scramble sequence by converting to cube state first"""
        if self.lib:
            try:
                # First convert scramble to cube state
                cube_state = self.scramble_to_cube_string(scramble)
                if cube_state == "000000000111111111222222222333333333444444444555555555":
                    # If conversion failed, use fallback method
                    return self._fallback_solve_scramble(scramble)
                
                # Now solve the cube state
                return self.solve(cube_state)
            except Exception as e:
                return f"Error: Scramble solving error - {str(e)}"
        else:
            return self._fallback_solve_scramble(scramble)
    ...
    def generate_scramble(self) -> str:
        ...
        if self.lib:
            try:
                ...
                if result:
                    scramble = self._safe_decode(result)
                    if scramble and all(c in "URFDLB'2 " for c in scramble):
                        return scramble
                    else:
                        print("Generated scramble contains invalid characters, using fallback")
    ...
    def is_valid_cube(self, cube_string: str) -> bool:
        ...
        if self.lib:
            try:
                ...
                # Use C++ validation if basic checks pass
    ...
    def scramble_to_cube_string(self, scramble: str) -> str:
        ...
        if self.lib:
            try:
                ...
                if result:
                    cube_state = self._safe_decode(result)
                    if cube_state and len(cube_state) == 54 and all(c in '012345' for c in cube_state):
                        return cube_state
                    else:
                        print("Invalid cube state from C++ library, using fallback")
                        return self._fallback_scramble_to_cube_string(scramble)
    ...
    def _fallback_solve_scramble(self, scramble: str) -> str:
        """Fallback scramble solving using move inversion"""
        try:
            move_inverses = {
                'U': "U'", "U'": 'U', 'U2': 'U2',
                'R': "R'", "R'": 'R', 'R2': 'R2',
    ...
    def _fallback_scramble_to_cube_string(self, scramble: str) -> str:
        """Fallback scramble to cube string conversion"""
        # For now, return solved cube state as fallback
        # In a real implementation, this would simulate the scramble moves
        return "000000000111111111222222222333333333444444444555555555"
...
if __name__ == "__main__":
    print("Testing Fixed In-House Kociemba Solver Wrapper...")
    ...
    print("Fixed In-house Kociemba wrapper test completed!")
